...
(function (window, undefined) {
  ...
  function resolve(ctxdata, env, expr, index) {
    ...
    if (typeof expr === "boolean" || typeof expr === "number" || !expr) {
      return expr;
    }
    ...
    function getWithFallback(id) {
      ...
      while ((loc = this.supportedLocales[cur])) {
        ...
        if (entry === undefined) {
          ...
          warning.call(
            this,
            new L10nError(id + " not found in " + loc, id, loc)
          );
          continue;
        }
        return entry;
  ...
  function inlineLocalization() {
    var script = document.documentElement.querySelector(
      'script[type="application/l10n"]' + '[lang="' + navigator.language + '"]'
    );
    if (!script) {
      return false;
    }
  ...
  function initResources() {
    var resLinks = document.head.querySelectorAll(
